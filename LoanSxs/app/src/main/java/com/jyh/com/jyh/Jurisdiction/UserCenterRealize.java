package com.jyh.com.jyh.Jurisdiction;
...
import com.jyh.com.jyh.Service.UpdataService;
...
//                Log.d("创建文件失败", "WRITE_EXTERNAL_STORAGE 权限");
    ...
    public void startPhotoAlbum(Context context) {
        if (!AppUtil.getInstance().existSDCard()) {
    ...
    public void getContactID(Activity activity, Handler mHandler, int booint) {
        ...
        cursor.close();
        if (AppUtil.getInstance().isFastDoubleClick()) {
        } else {
            if (booint == 1) {
            } else {
            }
